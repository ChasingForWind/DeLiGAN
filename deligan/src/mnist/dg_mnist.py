...
def Minibatch_Discriminator(input, num_kernels=100, dim_per_kernel=5, init=False, name='MD'):
    ...
    W = tf.multiply(theta, tf.expand_dims(tf.exp(log_weight_scale)/tf.sqrt(tf.reduce_sum(input_tensor=tf.square(theta),axis=0)),0))
    ...
    abs_dif = tf.multiply(tf.reduce_sum(input_tensor=tf.abs(tf.subtract(tf.expand_dims(activation,3),tf.expand_dims(tf.transpose(a=activation,perm=[1,2,0]),0))),axis=2),
                                                1-tf.expand_dims(tf.constant(np.eye(batchsize),dtype=np.float32),1))
    f = tf.reduce_sum(input_tensor=tf.exp(-abs_dif),axis=2)/tf.reduce_sum(input_tensor=tf.exp(-abs_dif))
    print(f.get_shape())
...
def fc_batch_norm(x, n_out, phase_train, name='bn'):
        ...
        batch_mean, batch_var = tf.nn.moments(x=x, axes=[0], name=name + '/fc_moments')
        ema = tf.train.ExponentialMovingAverage(decay=0.9)
        def mean_var_with_update():
            ema_apply_op = ema.apply([batch_mean, batch_var])
            with tf.control_dependencies([ema_apply_op]):
                return tf.identity(batch_mean), tf.identity(batch_var)
        mean, var = tf.cond(pred=phase_train, true_fn=mean_var_with_update, false_fn=lambda: (ema.average(batch_mean), ema.average(batch_var)))
...
def global_batch_norm(x, n_out, phase_train, name='bn'):
    ...
    batch_mean, batch_var = tf.nn.moments(x=x, axes=[0,1,2], name=name + '/moments')
    ema = tf.train.ExponentialMovingAverage(decay=0.9)
    def mean_var_with_update():
        ema_apply_op = ema.apply([batch_mean, batch_var])
        with tf.control_dependencies([ema_apply_op]):
...
def conv(x,  Wx, Wy,inputFeatures, outputFeatures, stridex=1, stridey=1, padding='SAME', transpose=False, name='conv'):
    ...
    conv = tf.nn.conv2d(input=x, filters=w, strides=[1,stridex,stridey,1], padding=padding) + b
    return conv
...
def discriminator(image, Reuse=False):
    with tf.compat.v1.variable_scope('disc', reuse=Reuse):
        ...
        bt = batch_norm(momentum=0.9, scale=True)
        h1 = lrelu(bt(conv(h0, 5, 5, df_dim,df_dim*2,stridex=2,stridey=2,name='d_h1_conv')))
        h2 = lrelu(bt(conv(h1, 3, 3, df_dim*2, df_dim*4, stridex=2, stridey=2,name='d_h2_conv')))
...
with tf.compat.v1.Session() as sess:
    ...
    d_loss_real = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(D_logit, tf.ones_like(D_logit)))
    d_loss_fake = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(D_fake_logit, tf.zeros_like(D_fake_logit)))

    sigma_loss = tf.reduce_mean(input_tensor=tf.square(zsig-1))/3    # sigma regularizer
    gloss = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(D_fake_logit, tf.ones_like(D_fake_logit)))
